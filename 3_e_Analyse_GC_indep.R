...
Resu_GC_Bin_both_file = paste(Resu_file,"/GC_Bin_analyse_both_indep",sep = "")
Resu_GC_Bin_indv_file = paste(Resu_file,"/GC_Bin_analyse_indv_indep",sep = "")
dir.create(Resu_GC_Bin_both_file)
dir.create(Resu_GC_Bin_indv_file)
...
for (n in Scen) {
  ...
  write.table(name_CL_GC_Bin,
              file = paste(Resu_GC_Bin_both_file,"/Data_output_CL_GC_both_indep_Bin_Scenario_",n,".csv",sep = ""),
              row.names = FALSE,
              col.names = FALSE,
              sep = ";")
  
  write.table(name_CL_GC_Bin,
              file = paste(Resu_GC_Bin_indv_file,"/Data_output_CL_GC_indv_indep_Bin_Scenario_",n,".csv",sep = ""),
              row.names = FALSE,
              col.names = FALSE,
              sep = ";")
}
...
for (n in Scen) {
  ...
  res <- foreach(i = 1:itt,
                 ...
                 .packages = c("stats",
                               ...
                               "here",
                               "geesmv",
                               "matrixcalc")) %dorng% fun_para_analyse_cluster_gcomp_indv_independence(itt_para = i,Scenario = Scenario_use,
                                                                                                       n = n,Data_file,Resu_file)
  ...
  res <- foreach(i = 1:itt,
                 ...
                 .packages = c("stats",
                               ...
                               "here",
                               "geesmv",
                               "matrixcalc")) %dorng% fun_para_analyse_cluster_gcomp_both_independence(itt_para = i,Scenario = Scenario_use,
                                                                                                       n = n,Data_file,Resu_file)
