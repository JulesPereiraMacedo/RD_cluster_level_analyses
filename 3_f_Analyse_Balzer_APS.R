...
Resu_TMLE_Balzer_APS_file = paste(Resu_file,"/TMLE_analyse_Balzer_APS",sep = "")

# dir.create(Resu_file)
dir.create(Resu_TMLE_Balzer_APS_file)
...
for (n in Scen) {
  ...
  write.table(name_CL_TMLE,
              file = paste(Resu_TMLE_Balzer_APS_file,"/Data_output_CL_TMLE_Balzer_APS_Scenario_",n,".csv",sep = ""),
              row.names = FALSE,
              col.names = FALSE,
              sep = ";")
...
for (n in Scen) {
  ...
  res <- foreach(i = 1:itt,
                 ...
                 .packages = c("stats","arm","gee","geepack","spind","doBy","doRNG","doParallel","dplyr","here","geesmv","matrixcalc")) %dorng% fun_para_analyse_cluster_TMLE_Balzer_APS(itt_para = i,Scenario = Scenario_use,n=n,Data_file=Data_file,Resu_file=Resu_file)
