...
Resu_GC_Bin_file = paste(Resu_file,"/GC_Bin_analyse_indv",sep = "")
...
for (n in Scen) {
  ...
  write.table(name_CL_GC_Bin,
              file = paste(Resu_GC_Bin_file,"/Data_output_CL_GC_indv_Bin_Scenario_",n,".csv",sep = ""),
              row.names = FALSE,
              col.names = FALSE,
              sep = ";")
}
...
for (n in Scen) {
  ...
  res <- foreach(i = 1:itt,
                 ...
                 .packages = c("stats","arm","gee","geepack","spind","doBy","doRNG","doParallel","dplyr","here","geesmv","matrixcalc")) %dorng% fun_cor_gcomp(i = i,Time,n,Base_file,Workspace_name,Data_file,Resu_file,cov_schem = "individual")
